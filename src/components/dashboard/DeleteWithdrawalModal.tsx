...
interface DeleteWithdrawalModalProps {
  ...
  investment: WithdrawalRow;
}

// ----------------------------------------------------------------------

export default function DeleteWithdrawalModal({ open, setOpen, investment }: DeleteWithdrawalModalProps) {
  ...
  const handleDelete = () => {
    setLoading(true);
    axios
      .delete(`/api/invest/${investment?._id}/`)
      .then(function (res) {
        setLoading(false);
        setOpen(false);
          ...
          <div className="flex min-h-full justify-center p-4 text-center items-center sm:p-0">
            ...
            >
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-[28rem] sm:p-6">
                ...
                <div className="bg-gray-50 px-4 py-3 sm:flex gap-5 sm:flex-row-reverse sm:px-6">
                  <LoadingButton
                    type="button"
                    color="error"
                    variant="contained"
